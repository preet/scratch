...
{
    if(argc != 3)   {
        std::cout << "Usage: #> ./wkt2bin inputfile.csv outputfile.dat\n";
        std::cout << " * Expect each line of input to contain a single WKT def\n";
        std::cout << " * The output file is a binary containing a single Geometry struct\n";
    ...
    outputBinFile.open(argv[2], std::ios::out | std::ios::binary);

    Vx myVx;
    std::vector<Vx> listVx;

    if(inputWktFile.is_open() && outputBinFile.is_open())
    {
...
//                myVx.x = 0; myVx.y = 0; myVx.z = 0; listVx.push_back(myVx);
        ...
        // (not sure if vector<> is contiguous)
        unsigned int k=0;
        double listVxVals[listVx.size()*3];
        for(size_t i=0; i < listVx.size(); i++)   {
            listVxVals[k] = listVx[i].x;   k++;
            listVxVals[k] = listVx[i].y;   k++;
            listVxVals[k] = listVx[i].z;   k++;
        }

        // write out to file in binary
        outputBinFile.write(reinterpret_cast<char*>(&listVxVals),sizeof(listVxVals));
        outputBinFile.close();

        //
        std::cout << "Info: sizeof(listVxVals): "
                  << sizeof(listVxVals) << std::endl;

        // test debug read in and dump
//        double listInVals[listVx.size()*3];
//        std::ifstream inputTest(argv[2], std::ios::in | std::ios::binary);
//        while(inputTest)   {
//            double myVal;
//            inputTest.read(reinterpret_cast<char*>(&myVal),sizeof(double));
//            std::cout << "#! " << myVal << std::endl;
//        }
//        inputTest.close();
    }

    std::cout << "Info: Saved " << listVx.size()*3 << " points\n";
