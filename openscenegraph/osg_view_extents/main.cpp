...
osg::ref_ptr<osg::AutoTransform> CreateLevelRingsNode()
{
...
osg::ref_ptr<osg::Group> BuildTilesNode(osg::Vec4 color)
{
...
int main(int argc, const char *argv[])
{
    ...
    // Create and add default camera frustum node
    gp_root->addChild(BuildFrustumNode(NULL,osg::Vec4(1,1,1,1)).get());

    // Create and add lod rings
    osg::ref_ptr<osg::AutoTransform> xf_rings = CreateLevelRingsNode();
    xf_rings->setName("rings");
    gp_root->addChild(xf_rings.get());

    // Create the tiles
    osg::ref_ptr<osg::Group> gp_tiles = BuildTilesNode(osg::Vec4(0.24,0.24,0.24,1.0));
    gp_root->addChild(gp_tiles.get());

    osgViewer::CompositeViewer viewer;

    // Create View 0 -- Just the tiles
    {
        osgViewer::View* view = new osgViewer::View;
        viewer.addView( view );

        view->setUpViewInWindow( 10, 10, 640, 480 );
        view->setSceneData( gp_tiles.get() );
        view->getCamera()->setClearColor(osg::Vec4(0.1,0.1,0.1,1.0));
        view->setCameraManipulator( new osgGA::TrackballManipulator );
    }

    // Create view 1 -- tiles + (the other) camera frustum + lod rings
    {
        osgViewer::View* view = new osgViewer::View;
        viewer.addView( view );
